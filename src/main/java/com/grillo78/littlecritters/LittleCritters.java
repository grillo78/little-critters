...
public class LittleCritters
{
    ...
    private void onBiomeLoad(BiomeLoadingEvent event){
        if(event.getCategory() == Biome.Category.FOREST)
            event.getSpawns().getSpawner(EntityClassification.AMBIENT).add(new MobSpawnInfo.Spawners(ModEntities.FIRE_FLY_ENTITY.get(),5, 2, 6));
    }
    ...
    private void doClientStuff(FMLClientSetupEvent event){
        RenderingRegistry.registerEntityRenderingHandler(ModEntities.FIRE_FLY_ENTITY.get(), new FireFlyRendererFactory());
    }
